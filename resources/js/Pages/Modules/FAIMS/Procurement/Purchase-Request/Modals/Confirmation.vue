<template>
  ...
  >
    <form class="customform">
      ...
      <div>
        ...
        <table class="table table-bordered align-middle text-center" style="width: 100%;">
            ...
            <tr  v-for="(bid_item, indexData) in bidsForAward" :key="indexData">
                ...
                <td>{{ formatCurrency(bid_item.item_bid_price * bid_item.item_quantity) }}</td>
      ...
      <!-- Ranking section -->
      <div class="mt-4" v-if="checkedBidsPrice">

        <div v-for="(group, groupIndex) in groupedUnawardedBids" :key="group.item_no" class="mb-4">
          <div>
          ...
          <table class="table table-bordered" style="width: 100%;">
            ...
            >
              <template #item="{ element, index }">
                <tr v-if="element.item_bid_price > 0">
                  ...
                  <td>{{ formatCurrency(element.item_bid_price * element.item_quantity) }}</td>
    ...
    <template #footer>
      ...
      <b-button @click="submit()" variant="success" block :disabled="!rankChecked">Yes</b-button>
    </template>
  </b-modal>
</template>
...
export default {
  ...
  methods: {
    ...
    sortData() {
      ...
      console.log(this.bids , 779);

      this.bids.forEach(bid => {
        bid.bid_items.forEach(bid_item => {
          const entry = {
            bid_item_id: bid_item.bid_item_id,
            item_no: bid_item.item_no,
            item_quantity: bid_item.item_quantity,
            item_unit_type: bid_item.item_unit_type,
            item_description: bid_item.item_description,
            supplier_id: bid_item.supplier.id,
            supplier: bid_item.supplier,
            total_cost: bid_item.item_quantity * bid_item.item_unit_cost,
            item_bid_price: bid_item.item_bid_price,
            total_bid_price: bid_item.item_bid_price * bid.item_quantity,
            technical_proposal: bid_item.technical_proposal,
            delivery_term: bid_item.delivery_term,
            status: bid_item.status_id,
            rank: 1,
            is_checked: bid_item.is_checked,
          };

          if (bid_item.is_checked) {
            entry.rank = 1;
            this.bidsForAward.push(entry);
          } else {
            this.bidsNotForAward.push(entry);
            if (!grouped[bid_item.item_no]) grouped[bid_item.item_no] = [];
            grouped[bid_item.item_no].push(entry);
          }
        });
      });
      ...
      // 🔽 Sort each group by lowest total bid price
       this.groupedUnawardedBids = Object.entries(grouped)
        ...
        .map(([item_no, bids]) => ({
          item_no,
          bids: bids
            .sort((a, b) => (a.item_bid_price * a.item_quantity) - (b.item_bid_price * b.item_quantity))
            .map((bid, index) => ({
              ...bid,
              rank: index + 2,
            })),
    ...
    submit() {
      const data = {
        ...
        action: this.purchase_request.status_id == 13 ? 'rebid' : 'bid',
        option: 'save_bid_for_award',
      };

      router.post('/faims/bids', data);
      this.hide();
    },
